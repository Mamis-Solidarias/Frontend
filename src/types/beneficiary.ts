...
let beneficiaryId: number = 0;

const createBeneficiary = ( familyId: string, firstName: string, lastName: string, sex: number, birthday: Date, documentId: string, type: string, isActive: boolean, comments?: string) : void => {
    beneficiaries.push({id: beneficiaryId.toString(), familyId, firstName, lastName, sex, birthday, documentId, type, isActive, comments});
    beneficiaryId += 1;
}
