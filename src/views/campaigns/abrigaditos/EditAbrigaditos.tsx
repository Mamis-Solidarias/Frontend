...
export default (props: EditAbrigaditosProps) => {
  ...
  useEffect(() => {
    dispatch(updateEditAbrigaditos({...abrigaditosSelector.editAbrigaditos, ...{description: dataEdition.description, provider: dataEdition.provider, fundraiserGoal: dataEdition.fundraiserGoal}}));
      ...
      <DialogContent>
        <Box>
          <TextField id='description' type='text'
                     ...
                     value={abrigaditosSelector.editAbrigaditos.description} onChange={(e: any) => dispatch(updateEditAbrigaditos({...abrigaditosSelector.editAbrigaditos, ...{description: e.target.value}}))}
                     fullWidth={true} variant='standard'
          />
          <TextField id='provider' type='text' sx={{py: '.3em'}}
                     inputProps={{pattern: '^.+$'}} label='Proveedor (opcional)'
                     placeholder='Catalan' value={abrigaditosSelector.editAbrigaditos.provider}
                     onChange={(e: any) => dispatch(updateEditAbrigaditos({...abrigaditosSelector.editAbrigaditos, ...{provider: e.target.value}}))}
                     fullWidth={true} variant='standard'
          />
          <TextField id='fundraiserGoal' type='number' sx={{py: '.3em'}}
                     inputProps={{pattern: '^.+$'}} label='Objetivo de Donaciones'
                     placeholder='12000.27' value={abrigaditosSelector.editAbrigaditos.fundraiserGoal}
                     onChange={(e: any) => dispatch(updateEditAbrigaditos({...abrigaditosSelector.editAbrigaditos, ...{fundraiserGoal: e.target.value}}))}
                     fullWidth={true} variant='standard'
          />
        </Box>
        ...
        <Box sx={{display: 'flex', flexDirection: 'row', justifyContent: 'center', marginTop: '1em'}}>
          <Button sx={{mx: '.25em'}} onClick={() => dispatch(updateOpenEditAbrigaditos(false))}>Cancelar</Button>
          <Button
            sx={{width: '60%', mx: '.25em'}}
            variant='contained'
            onClick={async () => await editAbrigaditos()}
            disabled={abrigaditosSelector.editAbrigaditos.fundraiserGoal <= 0 || dataBeneficiaries?.filteredBeneficiaries.nodes.length === 0}
          >
            Editar
          </Button>
        </Box>
