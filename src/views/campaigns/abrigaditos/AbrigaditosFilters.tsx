...
export default (props: { setAction: (action: Action) => void }) => {
  ...
  const setFilters = (field: keyof CampaignsFilters, value: any): any => dispatch(updateFiltersApplied({...abrigaditosSelector.filtersApplied,...{[field]: value}}));
  const applyFilters = () => {
  ...
  return <Card sx={{my: '2em', width: '100%', display: 'flex', flexDirection: 'column'}}>
    <CardHeader
      title='Filtros'
      action={
        <IconButton size='small' onClick={() => dispatch(updateOpenCollapse(!abrigaditosSelector.openFiltersCollapse))}>
