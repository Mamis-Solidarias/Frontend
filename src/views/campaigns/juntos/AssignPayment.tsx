...
export default (props: AssignPaymentProps) => {
  ...
  const assignPayment = async () => await createDonation({...campaignPaymentSelector.paymentData, ...{participantId: null}});
  ...
  return <Dialog
    open={campaignPaymentSelector.assignPayment}
    ...
    <DialogContent>
      <Autocomplete
        ...
        onChange={(e: any, newValue: { label: string; id: number }|null) => !!newValue && dispatch(updateDonorId(newValue.id))}
        value={campaignPaymentSelector.paymentData.donorId != -1 ? data.donors.nodes.find((donorFromData: Donor) => parseInt(donorFromData.id as string) === campaignPaymentSelector.paymentData.donorId).name : ''}
      ...
      <TextField
        ...
        variant='standard'
      >
        <MenuItem key={'ARS'} value={'ARS'}>Peso Argentino</MenuItem>
        <MenuItem key={'USD'} value={"USD"}>Dólar Estadounidense</MenuItem>
        <MenuItem key={'EUR'} value={'EUR'}>Euros</MenuItem>
      </TextField>
      <TextField
        id='amount'
        type='number'
        ...
        label='Cantidad'
        placeholder='200.0'
        value={campaignPaymentSelector.paymentData.amount > 0 ? campaignPaymentSelector.paymentData.amount: undefined}
