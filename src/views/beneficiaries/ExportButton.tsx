...
export default (props: ExportButtonProps) => {
  ...
  const translate = (entryName: keyof typeof nombresFiltros, value: any): string => {
    switch (entryName) {
      case 'type':
        return BENEFICIARY_TYPES[value as keyof typeof BENEFICIARY_TYPES];
      case 'gender':
    ...
    try {
      ...
      worksheet.addRow(
        Object.entries(filters).map(
          entry =>
            nombresFiltros[entry[0] as keyof typeof nombresFiltros] +
            ': ' +
      ...
      worksheet.addTable({
        ...
        rows: nodes.map((row: Beneficiary) => [
          row.familyId,
          row.dni,
          row.firstName + ' ' + row.lastName,
          GENDERS[row.gender as keyof typeof GENDERS],
          row.birthday,
          BENEFICIARY_TYPES[row.type as keyof typeof BENEFICIARY_TYPES],
          row.education?.school ? row.education?.school : '-',
          row.education?.year ? SCHOOL_YEARS[row.education?.year as keyof typeof SCHOOL_YEARS] : '-',
          row.education?.transportationMethod
            ? TRANSPORT_METHODS[row.education?.transportationMethod as keyof typeof TRANSPORT_METHODS]
            : '-',
          row.clothes?.shoeSize ? row.clothes?.shoeSize : '-',
          row.clothes?.shirtSize ? row.clothes?.shirtSize : '-',
          row.clothes?.pantsSize ? row.clothes?.pantsSize : '-',
          row.job?.title,
          row.health?.hasCovidVaccine ? 'Sí' : 'No',
          row.health?.hasMandatoryVaccines ? 'Sí' : 'No',
  ...
  return (
    <Button variant='contained' sx={{ mx: '.5em' }} onClick={async () => await exportToExcel()}>
      Exportar
    </Button>
  );
};
